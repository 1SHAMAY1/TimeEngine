...
public:
    ...
    // Spawns a particle emitter entity at world position with specified preset ("Fire", "Sparks", "Explosion", "Smoke",
    // "Rain")
    static Entity SpawnEmitterAtLocation(EntityManager &em, const TEVector &location, const TEString &preset = "Fire",
    ...
    static Entity SpawnHitImpactSparks(EntityManager &em, const TEVector &hitPoint, const TEVector &hitNormal,
    ...
    static Entity SpawnExplosion(EntityManager &em, const TEVector &location, uint32_t burstCount = 100,
                                 float speed = 8.0f, const TEColor &coreColor = TEColor(1.0f, 0.6f, 0.1f, 1.0f));

    // Spawns a rising smoke puff (muzzle flash, dust puff, landing impact)
    static Entity SpawnSmokePuff(EntityManager &em, const TEVector &location, float size = 0.8f, float lifetime = 2.0f,
    ...
    static Entity SpawnRainWeather(EntityManager &em, const TEVector &topCenter, float width = 12.0f,
                                   float rate = 160.0f);
