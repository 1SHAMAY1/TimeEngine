...
TE_REGISTER_MCP_TOOL(
    ...
    [](const TEString &paramsJson) -> TEString {
        auto editor = std::dynamic_pointer_cast<SoundStudio::SoundGraphAssetEditor>(AssetEditorRegistry::GetEditor("SoundGraph"));
        if (!editor)
        {
...
TE_REGISTER_MCP_TOOL(
    sound_add_node,
    "Add a DSP node (Oscillator, ADSR Envelope, Filter, Delay, Reverb, Gain, Mixer, BPM Clock, Audio Output) to the active sound graph.",
    "{\"type\":\"object\",\"properties\":{\"type\":{\"type\":\"string\"},\"x\":{\"type\":\"number\"},\"y\":{\"type\":\"number\"}},\"required\":[\"type\"]}",
    [](const TEString &paramsJson) -> TEString {
        auto editor = std::dynamic_pointer_cast<SoundStudio::SoundGraphAssetEditor>(AssetEditorRegistry::GetEditor("SoundGraph"));
        if (!editor)
...
TE_REGISTER_MCP_TOOL(
    ...
    "{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\"},\"duration\":{\"type\":\"number\"}},\"required\":[\"path\"]}",
    [](const TEString &paramsJson) -> TEString {
        auto editor = std::dynamic_pointer_cast<SoundStudio::SoundGraphAssetEditor>(AssetEditorRegistry::GetEditor("SoundGraph"));
        if (!editor)
            return "\"Error: SoundGraph editor is not active\"";
...
TE_REGISTER_MCP_TOOL(
    sound_preview_play,
    "Trigger or stop live audio synthesis preview in the Audio Studio.",
