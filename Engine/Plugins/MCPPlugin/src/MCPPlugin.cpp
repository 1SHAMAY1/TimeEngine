...
TE_PLUGIN_EXPORT IPlugin* CreatePluginInstance()
{
    return new MCPPlugin();
}

TE_PLUGIN_EXPORT void DestroyPluginInstance(IPlugin* plugin)
{
    delete plugin;
}
        ...
        else if (rpcMethod == "tools/list")
        {
            responseJson =
                ...
                "\"result\":{\"tools\":["
                  ...
                  "{"
                    "\"name\":\"create_sprite\","
                    "\"description\":\"Create a .tesprite asset file at the given relative path inside the project assets folder.\","
                    "\"inputSchema\":{"
                      "\"type\":\"object\","
                      "\"properties\":{"
                  ...
                  "{"
                    ...
                    "\"description\":\"Take screenshot of editor viewport, save to temp file, return absolute path, and clean up.\","
                    "\"inputSchema\":{\"type\":\"object\",\"properties\":{}}"
                  "},"
                  "{"
                  ...
                  "{"
                    ...
                    "\"inputSchema\":{"
                      "\"type\":\"object\","
                      "\"properties\":{"
                        "\"type\":{\"type\":\"string\",\"description\":\"Input event type: key_press, key_release, mouse_press, mouse_release\"},"
                  ...
                  "{"
                    ...
                    "\"inputSchema\":{"
                      ...
                      "\"properties\":{"
                        ...
                        "\"properties\":{\"type\":\"object\",\"description\":\"JSON object mapping component names to key-value variables\"}"
                      "},"
...
{
    if (toolName == "get_engine_info")       return Tool_GetEngineInfo();
    if (toolName == "get_scene_hierarchy")   return Tool_GetSceneHierarchy();
    if (toolName == "create_entity")         return Tool_CreateEntity(paramsJson);
    if (toolName == "destroy_entity")        return Tool_DestroyEntity(paramsJson);
    if (toolName == "create_sprite")         return Tool_CreateSprite(paramsJson);
    if (toolName == "create_directory")      return Tool_CreateDirectory(paramsJson);
    if (toolName == "delete_file_or_directory") return Tool_DeletePath(paramsJson);

    // NEW Tools
    if (toolName == "get_editor_modes")      return Tool_GetEditorModes();
    if (toolName == "set_editor_mode")       return Tool_SetEditorMode(paramsJson);
    if (toolName == "get_viewport_screenshot") return Tool_GetViewportScreenshot();
    if (toolName == "send_editor_input")     return Tool_SendEditorInput(paramsJson);
    if (toolName == "select_entity")         return Tool_SelectEntity(paramsJson);
    if (toolName == "set_entity_properties") return Tool_SetEntityProperties(paramsJson);
    if (toolName == "add_component")         return Tool_AddComponent(paramsJson);
    if (toolName == "delete_screenshot")     return Tool_DeleteScreenshot();
...
std::string MCPPlugin::Tool_GetSceneHierarchy()
{
    ...
    {
        if (!layer || layer->GetName() != "EditorLayer") continue;
...
std::string MCPPlugin::Tool_DestroyEntity(const std::string& paramsJson)
{
    ...
    if (id < 0) return "\"Error: Invalid or missing entity ID\"";
...
std::string MCPPlugin::Tool_SetEntityProperties(const std::string& paramsJson)
{
    ...
    for (Layer* layer : layers)
    {
        ...
        while (searchPos < propObj.length())
        {
            ...
            if (compValStart == std::string::npos) break;
            size_t compValEnd = compValStart;
            int braceCount = 1;
            while (braceCount > 0 && compValEnd < propObj.length() - 1)
            {
                compValEnd++;
                if (propObj[compValEnd] == '{') braceCount++;
                if (propObj[compValEnd] == '}')
                {
                    braceCount--;
                    if (braceCount == 0) { compValEnd++; break; }
                }
            }
            ...
            TE_CORE_INFO("[MCPPlugin Diagnostic] Entity ID: {0}, Total Components found: {1}", entityId, allComponents.size());
            ...
            for (auto* c : allComponents)
            {
                if (c)
                {
                    ...
                    TE_CORE_INFO("[MCPPlugin Diagnostic] - cClassName: {0}, raw_name: {1}", cClassName, typeid(*c).name());
            ...
            if (targetComponent)
            {
                if (compName == "TransformComponent")
                {
                    ...
                    TE_CORE_INFO("[MCPPlugin Diagnostic] TransformComponent: posStr={0}, rotStr={1}, scaleStr={2}", posStr, rotStr, scaleStr);

                    if (!posStr.empty())
                    {
                        std::stringstream ss(posStr);
                        ss >> targetComponent->Transform.Position.x 
                           >> targetComponent->Transform.Position.y 
                    ...
                    if (!rotStr.empty())
                    {
                        ...
                        ss >> targetComponent->Transform.Rotation.Pitch 
                           >> targetComponent->Transform.Rotation.Yaw 
                    ...
                    if (!scaleStr.empty())
                    {
                        ...
                        ss >> targetComponent->Transform.Scale.Scale.x 
                           >> targetComponent->Transform.Scale.Scale.y 
                ...
                else
                {
                    ...
                    if (metadata)
                    {
                        ...
                        for (const auto& propMeta : metadata->Properties)
                        {
                            ...
                            TE_CORE_INFO("[MCPPlugin Diagnostic] Checking property: {0}, extracted value: {1}", propMeta.Name, propVal);
...
void MCPPlugin::SendHttpResponse(uintptr_t socket, int statusCode,
                                  const std::string& contentType, const std::string& body,
                                  bool keepAlive)
{
    std::string statusText = "OK";
    if (statusCode == 202) statusText = "Accepted";
...
void MCPPlugin::CleanupSSEClients()
{
    ...
    m_SSEClients.erase(
        std::remove_if(m_SSEClients.begin(), m_SSEClients.end(),
                       [](const SSEClient& c) { return !c.Active; }),
        m_SSEClients.end());
}
...
std::string MCPPlugin::ExtractString(const std::string& json, const std::string& key)
{
    ...
    while (end < json.size())
    {
        if (json[end] == '\\') { end += 2; continue; } // skip escape
        if (json[end] == '"')  break;
        end++;
    }
