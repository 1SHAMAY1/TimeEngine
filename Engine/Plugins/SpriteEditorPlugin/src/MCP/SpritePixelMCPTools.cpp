...
TE_REGISTER_MCP_TOOL(
    sprite_pixel_draw,
    "Draw pixel or brush stroke onto the active layer.",
    "{\"type\":\"object\",\"properties\":{\"x\":{\"type\":\"integer\"},\"y\":{\"type\":\"integer\"},\"r\":{\"type\":\"number\"},\"g\":{\"type\":\"number\"},\"b\":{\"type\":\"number\"},\"a\":{\"type\":\"number\"},\"size\":{\"type\":\"integer\"},\"mirror\":{\"type\":\"boolean\"}},\"required\":[\"x\",\"y\"]}",
    [](const TEString &paramsJson) -> TEString {
        EditorMode *mode = EditorModeRegistry::GetActiveMode();
        if (!mode || TEString(mode->GetName()) != "Sprite Mode")
...
TE_REGISTER_MCP_TOOL(
    sprite_pixel_export_png,
    "Rasterize the sprite or animation spritesheet directly to a PNG file.",
    "{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\"},\"as_spritesheet\":{\"type\":\"boolean\"}},\"required\":[\"path\"]}",
    [](const TEString &paramsJson) -> TEString {
        EditorMode *mode = EditorModeRegistry::GetActiveMode();
        if (!mode || TEString(mode->GetName()) != "Sprite Mode")
            return "\"Error: Sprite Mode is not active\"";
...
TE_REGISTER_MCP_TOOL(
    ...
    [](const TEString &paramsJson) -> TEString {
        ...
        SpriteMode *sm = static_cast<SpriteMode *>(mode);
        TEString json = "{";
        json += "\"compiled\":" + TEString(sm->m_ScriptRuntime && sm->m_ScriptRuntime->IsValid() ? "true" : "false") + ",";
...
TE_REGISTER_MCP_TOOL(
    ...
    [](const TEString &paramsJson) -> TEString {
        ...
        for (size_t i = 0; i < codeStr.length(); ++i)
        {
            ...
            {
                if (codeStr[i + 1] == 'n') { unescaped += '\n'; ++i; }
                else if (codeStr[i + 1] == 't') { unescaped += '\t'; ++i; }
                else if (codeStr[i + 1] == '"') { unescaped += '"'; ++i; }
                else if (codeStr[i + 1] == '\\') { unescaped += '\\'; ++i; }
                else unescaped += codeStr[i];
            }
            else
            {
                unescaped += codeStr[i];
...
TE_REGISTER_MCP_TOOL(
    sprite_code_get_api,
    "Retrieve complete API documentation of all available TScript drawing, text, pixel art, animation, and color functions.",
    "{\"type\":\"object\",\"properties\":{}}",
    [](const TEString &paramsJson) -> TEString {
...
TE_REGISTER_MCP_TOOL(
    ...
    "{\"type\":\"object\",\"properties\":{\"total_frames\":{\"type\":\"integer\"},\"fps\":{\"type\":\"number\"},\"playing\":{\"type\":\"boolean\"},\"loop\":{\"type\":\"boolean\"},\"frame\":{\"type\":\"integer\"}}}",
    [](const TEString &paramsJson) -> TEString {
        EditorMode *mode = EditorModeRegistry::GetActiveMode();
        if (!mode || TEString(mode->GetName()) != "Sprite Mode")
            return "\"Error: Sprite Mode is not active\"";
