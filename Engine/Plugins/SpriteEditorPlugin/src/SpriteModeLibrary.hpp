...
public:
    static TEArray<ScriptPresetTemplate> GetPresetTemplates()
    {
        ...
        presets.Add({"Animated Vector Star", "Vector", "A smooth rotating vector star with glowing background ring.",
                     "// Animated Vector Star (Seamless 8-frame loop)\n"
                     "set_loop_frames(8)\n"
                     "var w = get_width()\n"
                     "var h = get_height()\n"
                     "var cx = w * 0.5\n"
                     ...
                     "\n"
                     "// Outer Pulsing Ring\n"
                     "var ring_r = w * 0.36 + sin(t) * (w * 0.02)\n"
                     ...
                     "var angle = t\n"
                     "var outer = w * 0.28 + sin(t * 2.0) * (w * 0.015)\n"
                     "var inner = w * 0.13\n"
                     "draw_star(cx, cy, 5, outer, inner, rgba(1.0, 0.85, 0.2, 1.0), angle)\n"
                     "\n"
                     "// Center core & label\n"
                     "draw_circle_filled(cx, cy, w * 0.045, WHITE)\n"
                     "draw_text_outlined(cx - 14, cy + h * 0.38, \"STAR\", YELLOW, BLACK, 1.0)\n"});

        // 2. Pixel Art Bouncing Slime
        presets.Add({"Pixel Bouncing Slime", "Pixel",
                     ...
                     "\n"
                     "var a = cycle_angle()\n"
                     "var bounce = sin(a) * 2.0\n"
                     "var squash = cos(a) * 1.0\n"
                     "\n"
                     "var sx = 8\n"
                     "var sy = 10 + bounce\n"
                     "var w = 5 - squash\n"
        ...
        presets.Add({"Neon Shield HUD", "Vector", "Futuristic neon radar HUD with rotating scanners and status text.",
                     "// Neon Shield HUD (Seamless Loop)\n"
                     "set_loop_frames(16)\n"
        ...
        presets.Add({"Pixel Walk Cycle", "Pixel", "A smooth 8-frame pixel art character walking animation.",
                     ...
                     "\n"
                     "var a = cycle_angle()\n"
                     "var leg_offset = sin(a) * 2.0\n"
                     "var arm_offset = cos(a) * 2.0\n"
                     "var bob = abs(sin(a)) * 1.0\n"
                     "\n"
        ...
        presets.Add({"Magic Fireball", "Vector", "Multi-layered glowing magical fireball with ember particles.",
