...
public:
    ...
    // Advanced Components & Lifecycle
    void AddRotation(uint32_t entityID, float speed, int direction = 1, int mode = 0);
    void AddOscillation(uint32_t entityID, const TEVector2 &axis, float amplitude, float frequency);
    void AddProjectile(uint32_t entityID, bool faceVelocity, float speed, float maxSpeed, float bounceFactor);
    void AddPhysicalMaterial(uint32_t entityID, float staticFriction, float dynamicFriction, float restitution);
    void AddForceField(uint32_t entityID, int type, float strength, float radius);
    void WakeBody(uint32_t entityID);
    void WakeTouching(uint32_t entityID);
    bool IsSleeping(uint32_t entityID);
