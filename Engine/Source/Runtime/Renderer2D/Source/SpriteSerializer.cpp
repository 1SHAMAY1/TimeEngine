...
bool SpriteSerializer::Deserialize(const TEString &filepath)
{
    ...
    bool success = TEFileSystem::ForEachLine(filepath,
                                          ...
                                          else if (line.StartsWith("TexturePath: "))
                                          {
                                              ...
                                              if (!texPath.IsEmpty())
                                              {
                                                  ...
                                                  if (!TEFileSystem::Exists(resolvedPath))
                                                  {
                                                      ...
                                                      TEString parentRel =
                                                          (filepath.GetParentPath() / texPath).LexicallyNormal();
    ...
    if (success)
    {
        TE_CORE_INFO("SpriteSerializer: Deserialized sprite '", m_Sprite->GetName(), "' from '", filepath, "' -> Texture: '", m_Sprite->GetTexturePath(), "'");
    }
    return success;
}
