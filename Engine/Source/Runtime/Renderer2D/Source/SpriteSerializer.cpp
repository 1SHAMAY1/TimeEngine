...
bool SpriteSerializer::Deserialize(const TEString &filepath)
{
    ...
    bool success = TEFileSystem::ForEachLine(
        ...
        [this, &filepath](const TEString &line)
        {
            ...
            else if (line.StartsWith("TexturePath: "))
            {
                ...
                if (!texPath.IsEmpty())
                {
                    TEString resolvedPath = texPath.LexicallyNormal();
                    if (!TEFileSystem::Exists(resolvedPath))
                    {
                        // 1. Check relative to .tesprite file directory
                        TEString parentRel = (filepath.GetParentPath() / texPath).LexicallyNormal();
                        ...
                        else
                        {
                            ...
                            TEString fileInSameDir =
                                (filepath.GetParentPath() / texPath.GetFilename()).LexicallyNormal();
                            ...
                            else if (Project::GetActive())
                            {
                                ...
                                TEString assetDirRel = (Project::GetAssetDirectory() / texPath).LexicallyNormal();
    ...
    if (success)
    {
        TE_CORE_INFO("SpriteSerializer: Deserialized sprite '", m_Sprite->GetName(), "' from '", filepath,
                     "' -> Texture: '", m_Sprite->GetTexturePath(), "'");
    }
    return success;
}
