...
void UITextBox::DrawSelf()
{
    ...
    if (m_CustomBgColor.has_value())
    {
        TimeGUI::PushStyleColor(TimeGUICol_FrameBg, m_CustomBgColor.value());
        pushedColors++;
    }
    if (m_CustomBorderColor.has_value())
    {
        TimeGUI::PushStyleColor(TimeGUICol_Border, m_CustomBorderColor.value());
        pushedColors++;
    }
    ...
    if (m_CustomCursorColor.has_value())
    {
        TimeGUI::PushStyleColor(TimeGUICol_InputTextCursor, m_CustomCursorColor.value());
        pushedColors++;
    }
    ...
    if (pushedColors > 0)
        TimeGUI::PopStyleColor(pushedColors);
    if (pushedVars > 0)
