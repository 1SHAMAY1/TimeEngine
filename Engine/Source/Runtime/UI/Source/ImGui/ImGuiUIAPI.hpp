...
/// Dear ImGui concrete UI backend implementation.
/// Sole location where ImGui vendor headers and symbols are used.
class ImGuiUIAPI final : public UIAPI
{
public:
    ImGuiUIAPI() = default;
    ~ImGuiUIAPI() override = default;

    UIBackendType GetType() const override { return UIBackendType::DearImGui; }
    ...
    bool SupportsDocking() const override { return true; }
    bool SupportsViewports() const override { return true; }

    UIIO &GetIO() override;
    UIStyle &GetStyle() override;
    void SetStyle(const UIStyle &style) override;
...
private:
    void *m_Context = nullptr; // ImGuiContext*
    bool m_Initialized = false;
    bool m_RenderInitialized = false;
    UIIO m_IO;
    UIStyle m_Style;
    TimeGUIPayload m_CurrentPayload;
    void SyncIOFromImGui();

    void SyncStyleToImGui();
