...
// Type aliases for independent UI types
using TimeGUIFont = UIFont;
using TimeGUIIO = UIIO;
using TimeGUIStyle = UIStyle;
...
TE_API bool BeginDragDropTarget();
TE_API const TimeGUIPayload *AcceptDragDropPayload(const TEString &type, int flags = 0);
