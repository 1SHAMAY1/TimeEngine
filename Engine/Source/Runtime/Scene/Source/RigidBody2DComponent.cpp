...
void RigidBody2DComponent::ApplyImpulse(const TEVector2 &impulse, const TEVector2 &worldPoint)
{
    if (GetRigidBodyType() == ERigidBodyType2D::Dynamic && m_InternalBody.InverseMass > 0.0f)
    {
        ...
        m_InternalBody.IsSleeping = false;
    }
}

void RigidBody2DComponent::WakeBody(PhysicsWorld *physicsWorld)
{
    m_InternalBody.IsSleeping = false;
    if (physicsWorld && m_InternalBody.m_VeloxEntityID != 0)
    {
        physicsWorld->WakeBody(m_InternalBody.m_VeloxEntityID);
    }
}

void RigidBody2DComponent::WakeTouching(PhysicsWorld *physicsWorld)
{
    m_InternalBody.IsSleeping = false;
    if (physicsWorld && m_InternalBody.m_VeloxEntityID != 0)
    {
        physicsWorld->WakeTouching(m_InternalBody.m_VeloxEntityID);
    }
}
