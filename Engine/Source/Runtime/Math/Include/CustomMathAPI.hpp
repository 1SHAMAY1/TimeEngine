...
/// High-performance Custom SIMD Math Backend (AVX2/SSE/NEON/Pure Intrinsics).
/// Bypasses third-party math headers (GLM) for vectorized linear algebra.
class TE_API CustomMathAPI final : public MathAPI
{
public:
    CustomMathAPI() = default;
    ~CustomMathAPI() override = default;

    MathBackendType GetType() const override { return MathBackendType::Custom; }
