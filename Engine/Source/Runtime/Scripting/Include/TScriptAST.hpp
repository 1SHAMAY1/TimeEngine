...
struct PropertySetNode : public ASTNode
{
    ...
    PropertySetNode(ExprNode obj, TEString f, TEString o, ExprNode val)
        : ASTNode(ASTNodeType::PropertySet), object(std::move(obj)), field(std::move(f)), op(std::move(o)),
          value(std::move(val))
    {
    }
};
...
struct UnaryOpNode : public ASTNode
{
    TEString op;
    ExprNode operand;
    UnaryOpNode(TEString o, ExprNode expr) : ASTNode(ASTNodeType::UnaryOp), op(std::move(o)), operand(std::move(expr))
    {
    }
};
...
struct BlockNode : public ASTNode
{
    ...
    BlockNode(TEArray<StmtNode> stmts = {}) : ASTNode(ASTNodeType::Block), statements(std::move(stmts)) {}
};
...
struct ForCStyleNode : public ASTNode
{
    ...
    ForCStyleNode(StmtNode i, ExprNode c, StmtNode s, TERef<BlockNode> b)
        : ASTNode(ASTNodeType::ForCStyle), init(std::move(i)), condition(std::move(c)), step(std::move(s)),
          body(std::move(b))
    {
    }
};
