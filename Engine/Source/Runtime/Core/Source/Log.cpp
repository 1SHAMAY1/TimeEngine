...
CustomizableLogger &Log::GetCoreLogger() { return *s_CoreLogger; }
CustomizableLogger &Log::GetClientLogger() { return *s_ClientLogger; }
bool Log::HasCoreLogger() { return s_CoreLogger != nullptr; }
bool Log::HasClientLogger() { return s_ClientLogger != nullptr; }
...
static TEString ResolveEngineLogsDirectory()
{
    // Try executable parent path first
    TEString exeDir = PlatformUtils::GetExecutablePath().GetParentPath();
    TEString current = exeDir;
    for (int i = 0; i < 5 && !current.IsEmpty(); ++i)
    {
        if (TEFileSystem::Exists(current / "Artifacts") || TEFileSystem::Exists(current / "Premake5.lua"))
        {
            TEString artifactsLogs = current / "Artifacts/Logs";
            TEFileSystem::CreateDirectories(artifactsLogs);
            return artifactsLogs;
        }
    ...
    // Fallback
    TEString fallback = "Artifacts/Logs";
    TEFileSystem::CreateDirectories(fallback);
    return fallback;
}
...
void Log::Init(bool logToFile, const TEString &file, bool silentBanner)
{
    ...
    TEString logDir = ResolveEngineLogsDirectory();
    TEString coreLogPath = logDir / ("Core_" + file);
    TEString clientLogPath = logDir / ("Client_" + file);

    s_CoreLogger = CreateScope<CustomizableLogger>(logToFile, coreLogPath.c_str());
    s_ClientLogger = CreateScope<CustomizableLogger>(logToFile, clientLogPath.c_str());
...
void Log::SetProjectLogDirectory(const TEString &projectLogDir)
{
    if (projectLogDir.IsEmpty())
        return;

    TEFileSystem::CreateDirectories(projectLogDir);
    TEString projLogPath = projectLogDir / "Client_TimeEngineLog.json";

    s_ClientLogger = CreateScope<CustomizableLogger>(true, projLogPath.c_str());
    s_ClientLogger->registerLevel("INFO", "\033[36m");
    s_ClientLogger->registerLevel("WARNING", "\033[35m");
    s_ClientLogger->registerLevel("ERROR", "\033[91m");
    s_ClientLogger->registerLevel("DEBUG", "\033[94m");
    s_ClientLogger->registerLevel("CRITICAL", "\033[41m");
    TE_CORE_INFO("Project logging routed to: ", projLogPath);
}
