...
{
    s_IsShutdown.store(true, std::memory_order_seq_cst);
    {
...
void ShortcutManager::AddListener(const TEString &listenerName, ShortcutListenerFn listener)
{
    if (listenerName.empty() || !listener)
        return;
    std::lock_guard<std::mutex> lock(s_ListenerMutex);
    if (s_IsShutdown.load(std::memory_order_relaxed))
        return;
...
{
    if (shortcutId.empty() || s_IsShutdown.load(std::memory_order_relaxed))
        return false;
...
{
    if (key == 0 || s_IsShutdown.load(std::memory_order_relaxed))
        return false;
    ...
    TEArray<Shortcut> matchingShortcuts;
    {
        std::lock_guard<std::mutex> lock(s_ShortcutMutex);
        auto &reg = GetRegistry();
        for (const auto &pair : reg)
        {
            const auto &sc = pair.second;
            if (!sc.Enabled)
                continue;
            ...
            matchingShortcuts.Add(sc);
        }
    }

    for (const auto &sc : matchingShortcuts)
    {
        bool handled = false;
        if (sc.Callback)
        {
