...
void SpriteAssetEditor::DrawEditor(EditorTab &tab)
{
    ...
    if (!sprite->GetTexture())
    {
        if (!curTexPath.empty())
        {
            TEString parentRel = (tab.AssetPath.GetParentPath() / curTexPath).LexicallyNormal();
            if (TEFileSystem::Exists(parentRel))
                sprite->SetTexturePath(parentRel);
            else
            {
                TEString fnRel = (tab.AssetPath.GetParentPath() / curTexPath.GetFilename()).LexicallyNormal();
                if (TEFileSystem::Exists(fnRel))
                    sprite->SetTexturePath(fnRel);
            }
        }

        // Auto-resolve matching image source with the same base name in asset directory
        if (!sprite->GetTexture())
        {
            TEString stem = tab.AssetPath.GetStem();
            TEString parentDir = tab.AssetPath.GetParentPath();
            TEString candidatePng = (parentDir / (stem + ".png")).LexicallyNormal();
            TEString candidateJpg = (parentDir / (stem + ".jpg")).LexicallyNormal();
            TEString candidateTetex = (parentDir / (stem + ".tetexture")).LexicallyNormal();

            if (TEFileSystem::Exists(candidatePng))
                sprite->SetTexturePath(candidatePng);
            else if (TEFileSystem::Exists(candidateJpg))
                sprite->SetTexturePath(candidateJpg);
            else if (TEFileSystem::Exists(candidateTetex))
                sprite->SetTexturePath(candidateTetex);
        }
    }

    TEString spriteTexBuf = sprite->GetTexturePath();
    ...
    TimeGUI::Spacing();
    if (TimeGUI::Button("Reimport Texture", TEVector2(140.0f, 0.0f)))
    {
        TEString tPath = sprite->GetTexturePath();
        if (!tPath.empty())
        {
            AssetManager::ReimportAsset(tPath);
            sprite->SetTexturePath(tPath);
            AssetEditorRegistry::MarkAssetDirty(tab.AssetPath, true);
        }
    }
    TimeGUI::SameLine();
    if (TimeGUI::Button("Reimport With New Source...", TEVector2(190.0f, 0.0f)))
    {
        ...
        {
            TEString tPath = sprite->GetTexturePath();
            if (tPath.EndsWith(".tetexture"))
            {
                AssetManager::ReimportAssetWithNewSource(tPath, filepath);
                sprite->SetTexturePath(tPath);
            }
            else
            {
                // Direct image link: copy new source and update path using asset stem
                TEString destDir = tab.AssetPath.GetParentPath();
                TEString assetStem = tab.AssetPath.GetStem().IsEmpty() ? sprite->GetName() : tab.AssetPath.GetStem();
                TEString destFile = (destDir / (assetStem + filepath.GetExtension())).LexicallyNormal();
                TEFileSystem::CopyFile(filepath, destFile, true);
                sprite->SetTexturePath(destFile);
            }
            AssetEditorRegistry::MarkAssetDirty(tab.AssetPath, true);
        }
    }
    ...
    TimeGUI::SameLine();
    float zoomPercent = s_SpriteZoom * 100.0f;
    if (TimeGUI::SliderFloat("Zoom", &zoomPercent, 20.0f, 500.0f, "%.0f%%"))
        s_SpriteZoom = zoomPercent / 100.0f;
    ...
    if (tex)
    {
        ...
        // Draw Studio-grade Checkerboard Backdrop
        float tileSize = 16.0f;
        uint32_t colDark = TIMEGUI_COL32(28, 30, 36, 255);
        uint32_t colLight = TIMEGUI_COL32(44, 48, 56, 255);
        drawList.AddRectFilled(canvasPos, TEVector2(canvasPos.x + fullW, canvasPos.y + fullH), colDark);

        for (float y = 0; y < fullH; y += tileSize)
        {
            for (float x = 0; x < fullW; x += tileSize)
            {
                ...
                if ((ix + iy) % 2 == 0)
                {
                    ...
                    float bx2 = (bx1 + tileSize > canvasPos.x + fullW) ? canvasPos.x + fullW : bx1 + tileSize;
                    float by2 = (by1 + tileSize > canvasPos.y + fullH) ? canvasPos.y + fullH : by1 + tileSize;
                    drawList.AddRectFilled(TEVector2(bx1, by1), TEVector2(bx2, by2), colLight);
                }
            }
        }

        // Draw cropped sprite image (only if GPU handle is valid)
        uintptr_t rendererID = (uintptr_t)tex->GetRendererID();
        if (rendererID != 0)
        {
            TimeGUI::Image((void *)rendererID, TEVector2(fullW, fullH), TEVector2(su0, sv0), TEVector2(su1, sv1));
        }
        else
        {
            TimeGUI::Dummy(TEVector2(fullW, fullH));
            TEVector2 textPos = TEVector2(canvasPos.x + 12.0f, canvasPos.y + (fullH * 0.5f) - 8.0f);
            drawList.AddText(textPos, TIMEGUI_COL32(230, 80, 80, 255), "Texture GPU Handle Unloaded (ID: 0)");
        }

        drawList.AddRect(canvasPos, TEVector2(canvasPos.x + fullW, canvasPos.y + fullH),
