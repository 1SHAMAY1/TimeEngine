...
#include "Widgets/UITileView.hpp"
...
ContentBrowserPanel::ContentBrowserPanel() : IEditorPanel("Content Browser")
{
    ...
    m_TileView->OnItemDragSource = [this](const UITileItem &item)
    {
        if (m_TileView->GetSelectedItems().Size() > 1 && m_TileView->IsItemSelected(item.ID))
        {
            TEArray<TEString> paths;
            for (const auto &selectedId : m_TileView->GetSelectedItems())
                paths.Add(selectedId);
            DragDropManager::BeginSourcePathArray(paths, TEString::FromInt((int)paths.Size()) + " Items");
        }
        else
        {
            DragDropManager::BeginSourceString(TE_DND_ASSET_PATH, item.ID, item.Title);
        }
    };
}
...
{
    TEString searchStarts[] = {Project::GetProjectDirectory(), TEFileSystem::GetCurrentWorkingDirectory()};
    for (const auto &start : searchStarts)
    {
        TEString curr = start;
        while (!curr.empty())
        {
            if (curr.GetFilename() == "Resources" && TEFileSystem::Exists(curr / "Editor"))
            {
                return TEFileSystem::GetAbsolutePath(curr);
            }
            TEString candidate = curr / "Resources";
            if (TEFileSystem::Exists(candidate) && TEFileSystem::Exists(candidate / "Editor"))
            {
                return TEFileSystem::GetAbsolutePath(candidate);
            }
            if (!curr.HasParentPath() || curr.GetParentPath() == curr)
                break;
...
void ContentBrowserPanel::OnTimeGUIRender(Ref<EditorLayer> editor)
{
    ...
    if (isAssets)
    {
        TimeGUI::PushStyleColor(TimeGUICol_Button, TEVector4(0.24f, 0.27f, 0.33f, 1.0f));
        TimeGUI::PushStyleColor(TimeGUICol_ButtonHovered, TEVector4(0.30f, 0.34f, 0.42f, 1.0f));
    }
    ...
    // Green + Import Button
    TimeGUI::PushStyleColor(TimeGUICol_Button, TEVector4(0.18f, 0.65f, 0.28f, 1.0f));
    TimeGUI::PushStyleColor(TimeGUICol_ButtonHovered, TEVector4(0.24f, 0.78f, 0.36f, 1.0f));
    TimeGUI::PushStyleColor(TimeGUICol_ButtonActive, TEVector4(0.14f, 0.55f, 0.22f, 1.0f));
    if (TimeGUI::Button("+ Import"))
    {
        AssetEditorRegistry::OpenImportDialog(m_CurrentDirectory);
    }
    ...
    float pathWidth = availWidth - searchBarWidth - 12.0f;
    if (pathWidth < 80.0f)
        pathWidth = 80.0f;
    ...
    if (m_SearchBar)
    {
        m_SearchBar->SetSize(TEVector2(searchBarWidth, 0.0f));
        m_SearchBar->Draw();
    }
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        if (isSelected)
        {
            TimeGUI::PushStyleColor(TimeGUI::TimeGUICol_Button, TEVector4(0.25f, 0.28f, 0.35f, 1.0f));
            TimeGUI::PushStyleColor(TimeGUI::TimeGUICol_ButtonHovered, TEVector4(0.32f, 0.36f, 0.44f, 1.0f));
        }
    ...
    if (m_TileView)
    {
        ...
        {
            TEString fullPath = m_CurrentDirectory / item.ID;
            if (m_SelectedPaths.Size() > 1 && m_SelectedPaths.Contains(fullPath))
            {
                DragDropManager::BeginSourcePathArray(m_SelectedPaths);
            }
            else
            {
                DragDropManager::BeginSourceString(TE_DND_ASSET_PATH, fullPath, "Drag " + item.Title);
            }
        };
        m_TileView->OnItemDropTarget = [this, editor](const UITileItem &item)
        {
            if (item.Subtitle == "Folder")
            {
                TEString folderPath = m_CurrentDirectory / item.ID;
                DragDropManager::ExecuteTarget(DragDropTargetContext::ContentBrowserFolder, &folderPath, editor);
            }
        };
    ...
    if (TimeGUI::BeginPopup("ContentBrowserContextMenu"))
    {
        ...
        if (hasSelection)
        {
            ...
            if (TimeGUI::MenuItem(deleteLabel, "Delete"))
            {
                ...
                if (m_TileView)
                {
                    for (const auto &id : m_TileView->GetSelectedItems())
                    {
                        ...
                        {
                            AssetEditorRegistry::CloseAsset(target);
                            EditorSaveManager::UnregisterSavable(target);
            ...
            // Reimport Actions for Selected Asset
            if (isSingleItem && !TEFileSystem::IsDirectory(m_SelectedPath))
            {
                auto editorForAsset = AssetEditorRegistry::GetEditorForPath(m_SelectedPath);
                if (editorForAsset && editorForAsset->SupportsReimport())
                {
                    if (TimeGUI::MenuItem("Reimport"))
                    {
                        AssetManager::ReimportAsset(m_SelectedPath);
                    }
                    if (TimeGUI::MenuItem("Reimport With New File..."))
                    {
                        TEString filter = AssetEditorRegistry::BuildImportFileDialogFilter();
                        TEString newSrc = PlatformUtils::OpenFile(filter.c_str());
                        if (!newSrc.IsEmpty())
                        {
                            AssetManager::ReimportAssetWithNewSource(m_SelectedPath, newSrc);
                        }
                    }
                    TimeGUI::Separator();
                }
        ...
        if (TimeGUI::BeginMenu("Create New"))
        {
            ...
            if (TimeGUI::BeginMenu("Gameplay"))
            {
                if (TimeGUI::MenuItem("Game Manager (.tegamemanager)"))
                {
                    auto gm = AssetManager::CreateFromPrototype(".tegamemanager");
                    if (gm)
                        gm->OnContentBrowserCreate(m_CurrentDirectory);
                }
                if (TimeGUI::MenuItem("Player Character (.teplayer)"))
                {
                    auto p = AssetManager::CreateFromPrototype(".teplayer");
                    if (p)
                        p->OnContentBrowserCreate(m_CurrentDirectory);
                }
                if (TimeGUI::MenuItem("Player Controller (.tecontroller)"))
                {
                    auto pc = AssetManager::CreateFromPrototype(".tecontroller");
                    if (pc)
                        pc->OnContentBrowserCreate(m_CurrentDirectory);
                }
                TimeGUI::EndMenu();
            }
        ...
        if (TimeGUI::MenuItem("Import Asset..."))
        {
            AssetEditorRegistry::OpenImportDialog(m_CurrentDirectory);
        }
