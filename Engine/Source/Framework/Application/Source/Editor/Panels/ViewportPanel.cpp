...
#include "ViewportContextMenu.hpp"
...
void ViewportPanel::OnTimeGUIRender(Ref<EditorLayer> editor)
{
    ...
    if (fb)
    {
        ...
        if (textureID != 0 && viewportPanelSize.x > 0.0f && viewportPanelSize.y > 0.0f)
        {
            ...
            DragDropManager::ExecuteTarget(DragDropTargetContext::Viewport, nullptr, editor);
            hasValidTexture = true;
        }
    }

    if (!hasValidTexture && viewportPanelSize.x > 0.0f && viewportPanelSize.y > 0.0f)
    {
        TimeGUI::Dummy(viewportPanelSize);
    }
    ...
    if (vpSize.x > 0.0f && vpSize.y > 0.0f)
    {
        ...
        {
            dl.AddRectFilled(vpMin, vpMin + vpSize, 0xFF181818);
        }
    }
