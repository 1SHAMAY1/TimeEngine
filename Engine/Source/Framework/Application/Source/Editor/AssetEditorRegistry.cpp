...
static std::mutex s_AssetEditorRegistryMutex;
...
void AssetEditorRegistry::Register(TERef<AssetEditor> editor)
{
    if (editor)
    {
        std::lock_guard<std::mutex> lock(s_AssetEditorRegistryMutex);
...
static bool s_ShowImportModal = false;
static bool s_OpenImportPopupTriggered = false;
...
void AssetEditorRegistry::OnTimeGUIRender()
{
    ...
    // Dynamic Import Asset Modal
    if (s_ShowImportModal)
    {
        if (s_OpenImportPopupTriggered)
        {
            TimeGUI::OpenPopup("Import Asset Settings");
        ...
        TEVector2 centerPos = TEVector2(displaySize.x * 0.5f, displaySize.y * 0.5f);
        TimeGUI::SetNextWindowPos(centerPos, TimeGUICond_Appearing, TEVector2(0.5f, 0.5f));
        TimeGUI::SetNextWindowSize(TEVector2(560.0f, 440.0f), TimeGUICond_Appearing);

        TimeGUIWindowFlags modalFlags = TimeGUIWindowFlags_AlwaysAutoResize | TimeGUIWindowFlags_NoCollapse;
        if (TimeGUI::BeginPopupModal("Import Asset Settings", nullptr, modalFlags))
        {
            TimeGUI::TextColored(TEVector4(0.20f, 0.85f, 0.40f, 1.0f), "Import 2D Asset into Project");
            TimeGUI::Separator();
            TimeGUI::Spacing();

            TimeGUI::TextDisabled("Source File:");
            TimeGUI::SameLine();
            TimeGUI::Text("%s", s_ActiveImportConfig.SourceFilePath.c_str());

            TimeGUI::TextDisabled("Destination Directory:");
            TimeGUI::SameLine();
            TimeGUI::Text("%s", s_ActiveImportConfig.TargetDirectory.c_str());

            TimeGUI::Spacing();
            TimeGUI::Separator();
            TimeGUI::Spacing();

            TimeGUI::Text("Target Asset Name:");
            TimeGUI::SetNextItemWidth(360.0f);
            TimeGUI::InputText("##ImportTargetAssetNameInput", s_ActiveImportConfig.TargetAssetName);

            TimeGUI::SameLine();
            TimeGUI::TextDisabled("%s", s_ActiveImportConfig.TargetExtension.c_str());

            TimeGUI::Spacing();
            TimeGUI::Separator();
            TimeGUI::Spacing();

            if (s_ActiveImportEditor)
            {
                TimeGUI::Text("Asset Specific Settings:");
                TimeGUI::Spacing();
                s_ActiveImportEditor->DrawImportSettings(s_ActiveImportConfig);
            }
            ...
            bool importClicked = TimeGUI::Button("Import", TEVector2(120.0f, 0.0f));
            TimeGUI::SameLine();
            if (TimeGUI::Button("Cancel", TEVector2(80.0f, 0.0f)))
            {
            ...
            if (importClicked)
            {
                ...
                if (AssetManager::ImportAsset(s_ActiveImportConfig, outCreatedPath))
                {
                    OpenAsset(outCreatedPath);
                    s_ShowImportModal = false;
                    TimeGUI::CloseCurrentPopup();
                }
            }
...
TERef<AssetEditor> AssetEditorRegistry::GetEditorForImportExtension(const TEString &sourceExt)
{
    for (auto &pair : GetEditorsMap())
    {
        if (pair.second && pair.second->CanImportExtension(sourceExt))
            return pair.second;
    }
    return nullptr;
}

TEString AssetEditorRegistry::BuildImportFileDialogFilter()
{
    TEArray<TEString> allExts;
    for (auto &pair : GetEditorsMap())
    {
        if (!pair.second)
            continue;
        for (const auto &ext : pair.second->GetSupportedImportExtensions())
        {
    ...
    if (allExts.IsEmpty())
    {
        return "Image Files (*.png;*.jpg;*.jpeg;*.tga;*.bmp)\0*.png;*.jpg;*.jpeg;*.tga;*.bmp\0All Files (*.*)\0*.*\0";
    }

    TEString patterns = "";
    for (size_t i = 0; i < allExts.Num(); ++i)
    {
        patterns += "*" + allExts[i];
        if (i + 1 < allExts.Num())
            patterns += ";";
    ...
    TEString filter = "Supported Asset Files (" + patterns + ")\0" + patterns + "\0All Files (*.*)\0*.*\0";
    return filter;
}

#include "Utils/PlatformUtils.hpp"

void AssetEditorRegistry::OpenImportDialog(const TEString &targetDirectory, const TEString &sourceFilePath)
{
    TEString source = sourceFilePath;
    if (source.IsEmpty())
    {
        TEString filter = BuildImportFileDialogFilter();
        source = PlatformUtils::OpenFile(filter.c_str());
    }

    if (source.IsEmpty() || !TEFileSystem::Exists(source))
        return;

    TEString ext = source.GetExtension();
    ...
    config.SourceFilePath = source;
    config.TargetDirectory = targetDirectory.IsEmpty() ? "Assets" : targetDirectory;
    config.TargetAssetName = source.GetStem();
    config.TargetExtension = editor->GetImportTargetExtension(ext);
    config.AssetType = editor->GetAssetType();

    editor->OnInitImportConfig(config);

    s_ActiveImportConfig = config;
    s_ActiveImportEditor = editor;
