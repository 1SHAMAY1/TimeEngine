...
void LogoLayer::OnTimeGUIRender()
{
    ...
    // Fade-in at start, fast fade-out at finish when plugins are loaded
    const float k_MinDisplayDuration = 0.2f;
    float alpha = 1.0f;
    if (m_Time < 0.15f)
    {
        alpha = m_Time / 0.15f;
    }
    else if (m_PluginsLoaded && m_Time >= k_MinDisplayDuration && (m_Time - m_FinishTime) > 0.1f)
    {
        float fadeElapsed = (m_Time - Max(m_FinishTime, k_MinDisplayDuration)) - 0.1f;
        alpha = Max(0.0f, 1.0f - (fadeElapsed / 0.15f));
    ...
    TimeGUI::SetNextWindowPos(screenPos);
    TimeGUI::SetNextWindowSize(screenSize);
    ...
    TimeGUIWindowFlags windowFlags = TimeGUIWindowFlags_NoDocking | TimeGUIWindowFlags_NoTitleBar |
                                     ...
                                     TimeGUIWindowFlags_NoNavFocus | TimeGUIWindowFlags_NoBackground;

    TimeGUI::Begin("##LogoLayerWindow", nullptr, windowFlags);
