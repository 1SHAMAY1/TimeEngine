...
void ProfilingLayer::UpdateMetrics()
{
    ...
    // Convert frameTime to milliseconds for graphs & historical tracking
    m_FrameTimeHistory.push_back(m_CurrentMetrics.frameTime * 1000.0f);
    ...
    // Calculate dynamic memory totals from OS process memory
#ifdef _WIN32
    PROCESS_MEMORY_COUNTERS_EX pmc;
    if (GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS *)&pmc, sizeof(pmc)))
    {
        float processPrivateHeapMB = (float)pmc.PrivateUsage / (1024.0f * 1024.0f);
        m_HeapHistory.push_back(processPrivateHeapMB);
        m_ClassAllocations["Process Private Heap"].className = "Process Private Heap";
        m_ClassAllocations["Process Private Heap"].count = 1;
        m_ClassAllocations["Process Private Heap"].sizeBytes = pmc.PrivateUsage;
        m_ClassAllocations["Process Private Heap"].isHeap = true;

        m_ClassAllocations["Working Set (Physical RAM)"].className = "Working Set (Physical RAM)";
        m_ClassAllocations["Working Set (Physical RAM)"].count = 1;
        m_ClassAllocations["Working Set (Physical RAM)"].sizeBytes = pmc.WorkingSetSize;
        m_ClassAllocations["Working Set (Physical RAM)"].isHeap = true;
    }
    else
    {
...
void ProfilingLayer::UpdateGPUMetrics()
{
    ...
    // Compute GPU VRAM usage from active textures and render target buffers
    float estimatedVramMB = 48.0f; // Swapchain & Render Targets baseline
    auto it = m_ClassAllocations.find("ShaderLibrary");
    if (it != m_ClassAllocations.end())
        estimatedVramMB += (float)it->second.sizeBytes / (1024.0f * 1024.0f);

    m_CurrentMetrics.gpuMemory = estimatedVramMB;
    if (m_VRAMTotal > 0)
        m_CurrentMetrics.vramUsage = (estimatedVramMB / (float)m_VRAMTotal) * 100.0f;
    else
        m_CurrentMetrics.vramUsage = std::min(100.0f, (estimatedVramMB / 4096.0f) * 100.0f);
}
                                 ...
                                 float minValue, float maxValue)
{
    ...
    float currentVal = data.back();
    float minVal = *std::min_element(data.begin(), data.end());
    float maxVal = *std::max_element(data.begin(), data.end());
    float avgVal = std::accumulate(data.begin(), data.end(), 0.0f) / (float)data.size();

    TimeGUI::TextColored(color, "%s: ", title.c_str());
    TimeGUI::SameLine();
    TimeGUI::TextColored(TEColor(0.95f, 0.96f, 0.98f, 1.0f), "%.1f", currentVal);
    TimeGUI::SameLine();
    TimeGUI::TextDisabled("(Avg: %.1f | Min: %.1f | Max: %.1f)", avgVal, minVal, maxVal);
    ...
    TimeGUI::SetCursorScreenPos(TEVector2(graphPos.x, graphPos.y + graphSize.y + 6));
    TimeGUI::Spacing();
}
                                   ...
                                   float maxValue, const TEString &label)
{
    ...
    // Draw background with subtle rounded border
    drawList.AddRectFilled(pos, TEVector2(pos.x + size.x, pos.y + size.y), IM_COL32(18, 20, 24, 255), 4.0f);

    // Draw grid lines
    for (int i = 0; i <= 4; i++)
    {
        float y = pos.y + (size.y * i / 4.0f);
        drawList.AddLine(TEVector2(pos.x, y), TEVector2(pos.x + size.x, y), IM_COL32(40, 44, 52, 200));
    }

    // Draw data line & shaded area
    if (data.size() > 1)
    {
        ...
        uint32_t fillCol =
            IM_COL32((int)(color.x * 255), (int)(color.y * 255), (int)(color.z * 255), (int)(color.w * 45));
        uint32_t lineCol =
            IM_COL32((int)(color.x * 255), (int)(color.y * 255), (int)(color.z * 255), (int)(color.w * 255));

        // Draw translucent filled strip under curve
        for (size_t i = 1; i < data.size(); i++)
        {
            float x1 = pos.x + (size.x * (i - 1) / (data.size() - 1));
            float val1 = std::clamp(data[i - 1], minValue, maxValue);
            float y1 = pos.y + size.y - (size.y * (val1 - minValue) / range);

            float x2 = pos.x + (size.x * i / (data.size() - 1));
            float val2 = std::clamp(data[i], minValue, maxValue);
            float y2 = pos.y + size.y - (size.y * (val2 - minValue) / range);

            TEVector2 p1(x1, y1);
            TEVector2 p2(x2, y2);
            TEVector2 p3(x2, pos.y + size.y);
            TEVector2 p4(x1, pos.y + size.y);
            drawList.AddQuadFilled(p1, p2, p3, p4, fillCol);

            // Draw crisp top curve
            drawList.AddLine(p1, p2, lineCol, 2.0f);
        }
    }

    // Draw card border
    drawList.AddRect(pos, TEVector2(pos.x + size.x, pos.y + size.y), IM_COL32(60, 66, 78, 200), 4.0f);
}
