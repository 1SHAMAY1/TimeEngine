...
namespace TE {
    ...
    // -------------------------------------------------------------------------
    static ID3D10Blob* CompileHLSL(const std::string& src,
    ...
    void DirectX11Shader::Bind() const {
        ...
        if (m_Dirty) FlushConstantBuffer();
    ...
    void DirectX11Shader::FlushConstantBuffer() const {
        DX11Context& ctx = DX11Context::Get();
        if (!ctx.DeviceContext || !m_ConstantBuffer) return;
    ...
    void DirectX11Shader::SetUniform1f(const std::string& name, float value) {
        ...
        if (offset < 256) m_CBData.data[offset] = value;
        m_Dirty = true;
    }
    ...
    void DirectX11Shader::SetUniform1i(const std::string& name, int value) {
        ...
        if (offset < 256) m_CBData.data[offset] = static_cast<float>(value);
        m_Dirty = true;
    }
