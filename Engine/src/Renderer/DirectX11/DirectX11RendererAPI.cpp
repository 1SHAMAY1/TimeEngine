...
namespace TE {
    ...
    GraphicsAPI RendererAPI::GetAPI() {
        return RendererContext::GetAPI();
    }

    std::unique_ptr<RendererAPI> RendererAPI::Create() {
        switch (RendererContext::GetAPI()) {
            case GraphicsAPI::None:       return nullptr;
            case GraphicsAPI::OpenGL:     return std::make_unique<OpenGLRendererAPI>();
#if defined(TE_PLATFORM_MOBILE)
            case GraphicsAPI::OpenGLES:   return std::make_unique<OpenGLESRendererAPI>();
#else
            case GraphicsAPI::OpenGLES:   return nullptr;
#endif
            case GraphicsAPI::Vulkan:     return std::make_unique<VulkanRendererAPI>();
            case GraphicsAPI::DirectX11:  return std::make_unique<DirectX11RendererAPI>();
        }
        return nullptr;
    }
    ...
    void DirectX11RendererAPI::InitWithWindow(void* hwnd, uint32_t width, uint32_t height) {
        ...
        if (FAILED(hr)) {
            std::cerr << "[DirectX11] D3D11CreateDeviceAndSwapChain failed: 0x"
                      << std::hex << hr << std::endl;
            return;
        }
    ...
    void DirectX11RendererAPI::ReleaseRenderTargets() {
        ...
        if (ctx.DepthStencilTex)  { ctx.DepthStencilTex->Release();  ctx.DepthStencilTex  = nullptr; }
    ...
    void DirectX11RendererAPI::SetViewport(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
        ...
        if (!DX11Context::Get().DeviceContext) return;
    ...
    void DirectX11RendererAPI::Clear() {
        ...
        if (ctx.DepthStencilView)
            ctx.DeviceContext->ClearDepthStencilView(ctx.DepthStencilView,
    ...
    std::string DirectX11RendererAPI::GetGPURenderer()   { return m_GPUDescription.empty() ? "Direct3D 11 Hardware" : m_GPUDescription; }
    ...
    void DirectX11RendererAPI::ReadPixelsRGBA(int x, int y, int width, int height, void* outPixels) {
        DX11Context& ctx = DX11Context::Get();
        if (!ctx.Device || !ctx.DeviceContext || !ctx.SwapChain) return;

        ID3D11Texture2D* backBuffer = nullptr;
        if (FAILED(ctx.SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D),
            reinterpret_cast<void**>(&backBuffer)))) return;
        ...
        if (SUCCEEDED(ctx.DeviceContext->Map(stagingTex, 0, D3D11_MAP_READ, 0, &mapped))) {
            ...
            for (int row = 0; row < height; ++row) {
                ...
                if (srcRow < 0 || srcRow >= static_cast<int>(desc.Height)) continue;
    ...
    void DirectX11RendererAPI::SetBlendFuncSeparate(BlendFactor srcRGB, BlendFactor dstRGB,
