...
{
    ImGui::ImageWithBg(ImTextureRef((ImTextureID)userTextureId), ToImVec2(size), ToImVec2(uv0), ToImVec2(uv1),
...
bool ImGuiUIAPI::ImageButton(const TEString &strId, void *userTextureId, const TEVector2 &size, const TEVector2 &uv0,
                             const TEVector2 &uv1)
{
    return ImGui::ImageButton(strId.c_str(), (ImTextureID)userTextureId, ToImVec2(size), ToImVec2(uv0), ToImVec2(uv1));
}
...
void ImGuiUIAPI::DrawListAddLine(void *drawList, const TEVector2 &p1, const TEVector2 &p2, unsigned int color,
                                 float thickness)
{
    auto *dl = static_cast<ImDrawList *>(drawList ? drawList : ImGui::GetWindowDrawList());
    if (dl)
