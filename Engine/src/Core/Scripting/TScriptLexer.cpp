...
TScriptLexer::TScriptLexer(TEString source)
...
bool TScriptLexer::IsAtEnd() const {
...
TEArray<TScriptToken> TScriptLexer::Tokenize() {
    ...
    while (!IsAtEnd()) {
        ...
        switch (c) {
            case '(': tokens.push_back(MakeToken(TScriptTokenType::LParen)); break;
            case ')': tokens.push_back(MakeToken(TScriptTokenType::RParen)); break;
            case '{': tokens.push_back(MakeToken(TScriptTokenType::LBrace)); break;
            case '}': tokens.push_back(MakeToken(TScriptTokenType::RBrace)); break;
            case '[': tokens.push_back(MakeToken(TScriptTokenType::LBracket)); break;
            case ']': tokens.push_back(MakeToken(TScriptTokenType::RBracket)); break;
            case ',': tokens.push_back(MakeToken(TScriptTokenType::Comma)); break;
            case '.': tokens.push_back(MakeToken(TScriptTokenType::Dot)); break;
            case ';': tokens.push_back(MakeToken(TScriptTokenType::Semicolon)); break;
            case ':': tokens.push_back(MakeToken(TScriptTokenType::Colon)); break;
            case '%': tokens.push_back(MakeToken(TScriptTokenType::Percent)); break;

            case '+':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::PlusEq));
                else tokens.push_back(MakeToken(TScriptTokenType::Plus));
                break;
            case '-':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::MinusEq));
                else tokens.push_back(MakeToken(TScriptTokenType::Minus));
                break;
            case '*':
                if (Match('*')) tokens.push_back(MakeToken(TScriptTokenType::StarStar));
                else if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::StarEq));
                else tokens.push_back(MakeToken(TScriptTokenType::Star));
                break;
            case '/':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::SlashEq));
                else tokens.push_back(MakeToken(TScriptTokenType::Slash));
                break;

            case '!':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::Neq));
                else tokens.push_back(MakeToken(TScriptTokenType::Bang));
                break;
            case '=':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::Eq));
                else tokens.push_back(MakeToken(TScriptTokenType::Assign));
                break;
            case '<':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::Leq));
                else tokens.push_back(MakeToken(TScriptTokenType::Lt));
                break;
            case '>':
                if (Match('=')) tokens.push_back(MakeToken(TScriptTokenType::Geq));
                else tokens.push_back(MakeToken(TScriptTokenType::Gt));
                break;
            case '&':
                if (Match('&')) tokens.push_back(MakeToken(TScriptTokenType::AndAnd));
                else tokens.push_back(MakeErrorToken("Unexpected character '&'"));
                break;
            case '|':
                if (Match('|')) tokens.push_back(MakeToken(TScriptTokenType::PipePipe));
                else tokens.push_back(MakeErrorToken("Unexpected character '|'"));
                break;
            default:
