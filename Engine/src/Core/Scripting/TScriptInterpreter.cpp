...
TEMap<TEString, TScriptValue> TScriptInterpreter::SnapshotVariables() const { return m_Globals; }

void TScriptInterpreter::RestoreVariables(const TEMap<TEString, TScriptValue> &snapshot) { m_Globals = snapshot; }
...
TScriptValue TScriptInterpreter::GetProperty(const ExprNode &target, const TEString &field)
{
    ...
    if (base.IsVec2())
    {
        ...
        if (field == "x")
            return TScriptValue::Number(v.x);
        if (field == "y")
            return TScriptValue::Number(v.y);
    }
    return TScriptValue::Nil();
}

void TScriptInterpreter::SetProperty(const ExprNode &target, const TEString &field, const TEString &op,
                                     const TScriptValue &val)
{
...
TScriptValue TScriptInterpreter::EvalExpr(const ExprNode &node)
{
    ...
    case ASTNodeType::BinaryOp:
    {
        ...
        if (bin->op == "/")
            return TScriptValue::Number(lhs.AsNumber() / rhs.AsNumber());
        if (bin->op == "%")
            return TScriptValue::Number(std::fmod(lhs.AsNumber(), rhs.AsNumber()));
        if (bin->op == "**")
