...
bool SceneSerializer::Deserialize(const std::filesystem::path &filepath)
{
    ...
    for (const auto& l : lines) {
        if (l.find("  - Entity: ") != std::string::npos) {
            ...
            if (!tagComp) tagComp = entityManager.AddComponent<TagComponent>(currentEntity);
            if (tagComp) tagComp->Tag = tagName;
        }
        else if (l.find("    ") == 0 && l.find(":") != std::string::npos && l.find("  - ") == std::string::npos && l.find("      ") != 0) {
            ...
            if (registryMap.count(currentComponentName)) {
                ...
                if (!currentComponent) {
                    currentComponent = registryMap.at(currentComponentName).Factory(&entityManager, currentEntity.GetID());
                }
            } else {
                currentComponent = nullptr;
            }
