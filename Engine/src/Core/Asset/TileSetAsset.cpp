...
TileSetAsset::TileSetAsset(const TEString &name)
    : m_Name(name)
{
}
...
bool TileSetAsset::LoadFromFile(const TEString &path)
{
    ...
    return TEFileSystem::ForEachLine(path, [this](const TEString &line) {
        ...
        {
            m_TextureAtlasHandle = static_cast<AssetHandle>(trimmed.Mid(7).Trim().ToInt64());
        }
...
bool TileSetAsset::SaveToFile(const TEString &path)
{
    ...
    for (size_t i = 0; i < m_Tiles.Num(); ++i)
    {
        ...
        file << "Tile: " << t.TileID << " " << t.UVRect.x << " " << t.UVRect.y << " "
             << t.UVRect.z << " " << t.UVRect.w << " " << (t.IsCollidable ? 1 : 0) << " " << t.Tag.c_str() << "\n";
    }
    return true;
}
