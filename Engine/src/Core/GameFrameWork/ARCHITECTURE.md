...
```mermaid
flowchart TD
    TObject["TObject (Base Object)"] --> TComponent["TComponent (ECS Component Base)"]
    TComponent --> Transform["Transform (TETransform)"]
    TComponent --> Geometry["Geometry & Shadows (ContainsPoint, GetWorldVertices)"]
    TComponent --> Inspector["Inspector GUI (OnDrawInspector)"]
```
...
### 3. `GameplayUtils` Object Creation

#### `GameplayUtils::CreateObject<T>(args...)`
- **When Used**: Instantiates standalone `TObject`-derived instances into the global object pool (`GetObjectPool()`).

```cpp
auto& myObj = GameplayUtils::CreateObject<MyCustomObject>("GameManager");
```
...
- [Scene & ECS Architecture](../Scene/ARCHITECTURE.md) — Documentation for `EntityManager` and component lifecycle.
- [Utils & Math Architecture](../../Utils/ARCHITECTURE.md) — Documentation for `TETransform` and 2D vector math primitives.
