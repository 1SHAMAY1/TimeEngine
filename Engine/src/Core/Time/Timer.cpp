...
Timer::TimerRef Timer::NextFrame(Callback callback) { return SetFrames(1, callback, false); }

void Timer::Clear(const TimerRef &ref) { s_Timers.erase(ref.id); }
...
void Timer::Shutdown() { s_Timers.clear(); }
