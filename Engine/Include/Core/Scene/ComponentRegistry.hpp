...
namespace TE {
    ...
    public:
        ...
        template <typename T>
        bool RegisterComponent(const std::string& className, const std::string& displayName) {
            auto& meta = m_Components[className];
            meta.ClassName = className;
            meta.DisplayName = displayName;
            meta.TypeIndex = std::type_index(typeid(T));
            meta.Factory = [](EntityManager* em, EntityID id) {
                return (TComponent*)em->AddComponent<T>(id);
            };
        ...
        bool RegisterProperty(const std::string& className, const std::string& propName, const std::string& displayName, Type Class::* member, std::function<bool(void*)> condition = nullptr) {
            auto& meta = m_Components[className];
            for (const auto& p : meta.Properties) {
                if (p.Name == propName) return true;
            }
        ...
        bool RegisterEnumProperty(const std::string& className, const std::string& propName, const std::string& displayName, EnumType Class::* member, const std::string& enumName) {
            auto& meta = m_Components[className];
            for (const auto& p : meta.Properties) {
                if (p.Name == propName) return true;
            }
                ...
                [member, enumName, propName](void* instance, const std::string& label) {
                    ...
                    if (enumMeta) {
                        ...
                        for (const auto& pair : enumMeta->Values) {
                            if (pair.second == *val) { current = pair.first.c_str(); break; }
                        }
                        ...
                        if (ImGui::BeginCombo(imguiLabel.c_str(), current)) {
                            for (const auto& pair : enumMeta->Values) {
                                ...
                                // Use ### with value to ensure unique ID even if names match (or are duplicated)
                                std::string selectableId = pair.first + "###" + std::to_string(pair.second);
        ...
        bool RegisterEnum(const std::string& name, const std::vector<std::pair<std::string, E>>& values) {
            ...
            if (!meta.Values.empty()) return true; // Already registered
        ...
        const std::map<std::string, ComponentMetadata>& GetComponents() const {
            return m_Components;
        }
        ...
        const ComponentMetadata* GetMetadata(std::type_index type) const {
            ...
            if (itN != m_TypeToName.end()) return GetMetadata(itN->second);
            return nullptr;
        }
        ...
        bool RegisterEntityPreset(const std::string& name, const std::string& category, EntityPresetFactory factory) {
            ...
            std::sort(m_EntityPresets.begin(), m_EntityPresets.end(), [](const EntityPreset& a, const EntityPreset& b) {
        ...
        const std::vector<EntityPreset>& GetEntityPresets() const {
            return m_EntityPresets;
        }

        bool MarkInternal(const std::string& className) {
            auto it = m_Components.find(className);
            if (it != m_Components.end()) it->second.IsInternal = true;
            return true;
        }
    ...
    struct TEComponentInternalRegistrar {
        TEComponentInternalRegistrar(const char* className) {
            ::TE::ComponentRegistry::Get().MarkInternal(className);
        }
    };
...
#define T_REGISTER_PROPERTY(Class, Type, Var, DName) \
    inline static bool s_##Class##_##Var##_Reg = ::TE::ComponentRegistry::Get().RegisterProperty<Class, Type>(#Class, #Var, DName, &Class::Var);

#define T_REGISTER_PROPERTY_COND(Class, Type, Var, DName, Cond) \
    inline static bool s_##Class##_##Var##_Reg = ::TE::ComponentRegistry::Get().RegisterProperty<Class, Type>(#Class, #Var, DName, &Class::Var, Cond);

#define T_REGISTER_ENUM_PROPERTY(Class, EnumType, Var, DName) \
    inline static bool s_##Class##_##Var##_Reg = ::TE::ComponentRegistry::Get().RegisterEnumProperty<Class, EnumType>(#Class, #Var, DName, &Class::Var, #EnumType);
...
#define T_COMPONENT_INTERNAL(Class) \
    inline static ::TE::TEComponentInternalRegistrar s_##Class##_InternalReg(#Class);
