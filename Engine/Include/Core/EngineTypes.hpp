...
template <typename T, typename E = std::string> class TEResult
{
private:
    bool m_HasValue;
    union
...
public:
    ...
    [[nodiscard]] constexpr T ValueOr(T fallback) const { return m_HasValue ? m_Value : fallback; }
...
namespace TE::Detail
{
template <typename T> struct IsSmartRef : std::false_type
{
};
template <typename T> struct IsSmartRef<std::shared_ptr<T>> : std::true_type
{
};

template <typename T> struct IsSmartScope : std::false_type
{
};
template <typename T, typename D> struct IsSmartScope<std::unique_ptr<T, D>> : std::true_type
{
};

template <typename From, typename To> struct IsLosslessNumeric
{
...
// ============================================================================
template <typename T> class TEInt
{
    static_assert(std::is_integral_v<T> && !std::is_same_v<T, bool>,
                  "TEInt<T> requires an integral type (excluding bool).");
...
public:
    ...
    constexpr TEInt &operator+=(const TEInt &other) noexcept
    {
    ...
    [[nodiscard]] constexpr TE::TEOption<TEInt<T>> CheckedMul(TEInt other) const noexcept
    {
        ...
        if constexpr (std::is_signed_v<T>)
        {
            if (m_Value > 0)
            {
                if (other.m_Value > 0 && m_Value > std::numeric_limits<T>::max() / other.m_Value)
                    return TE::TENone;
                if (other.m_Value < 0 && other.m_Value < std::numeric_limits<T>::min() / m_Value)
                    return TE::TENone;
            }
            else
            {
                if (other.m_Value > 0 && m_Value < std::numeric_limits<T>::min() / other.m_Value)
                    return TE::TENone;
                if (other.m_Value < 0 && other.m_Value < std::numeric_limits<T>::max() / m_Value)
                    return TE::TENone;
            }
        }
        else
    ...
    [[nodiscard]] constexpr TE::TEOption<TEInt<T>> CheckedDiv(TEInt other) const noexcept
    {
        ...
        if constexpr (std::is_signed_v<T>)
        {
            if (m_Value == std::numeric_limits<T>::min() && other.m_Value == -1)
                return TE::TENone;
        }
        return TEInt(m_Value / other.m_Value);
    }
...
// ============================================================================
template <typename T> class TEFloat
{
...
public:
    ...
    constexpr explicit(!std::is_same_v<T, OtherFloat> && sizeof(OtherFloat) > sizeof(T))
        TEFloat(OtherFloat value) noexcept
        : m_Value(static_cast<T>(value))
    {
    }
    ...
    constexpr TEFloat &operator+=(const TEFloat &other) noexcept
    {
        m_Value += other.m_Value;
        return *this;
    }
...
// ============================================================================
template <typename E> class TEEnum
{
...
public:
    ...
    [[nodiscard]] constexpr bool HasAnyFlags(TEEnum flags) const noexcept { return (m_Value & flags.m_Value) != 0; }
    ...
    constexpr TEEnum &operator|=(TEEnum other) noexcept
    {
        m_Value |= other.m_Value;
        return *this;
    }
...
// ============================================================================
template <typename T> class TEStruct
{
...
public:
    ...
    template <typename... Args> constexpr explicit TEStruct(Args &&...args) : m_Data(std::forward<Args>(args)...) {}
    ...
    template <typename Fn> constexpr void Mutate(Fn &&fn) { std::forward<Fn>(fn)(m_Data); }

    [[nodiscard]] constexpr TEStruct Clone() const { return TEStruct(m_Data); }
};

// ============================================================================
// 7. TEClass<T> - Smart Class / Metaclass & Instance Wrapper
// ============================================================================
template <typename T> class TEClass
{
...
public:
    ...
    [[nodiscard]] static constexpr size_t GetSize() noexcept { return sizeof(T); }

    [[nodiscard]] static constexpr size_t GetAlign() noexcept { return alignof(T); }

    template <typename Base> [[nodiscard]] static constexpr bool IsChildOf() noexcept
    {
        return std::is_base_of_v<Base, T>;
    }

    // Smart Factory Instantiation
    template <typename... Args> [[nodiscard]] static std::shared_ptr<T> CreateRef(Args &&...args)
    {
        return std::make_shared<T>(std::forward<Args>(args)...);
    }

    template <typename... Args> [[nodiscard]] static std::unique_ptr<T> CreateScope(Args &&...args)
    {
        return std::make_unique<T>(std::forward<Args>(args)...);
    }

    template <typename... Args> [[nodiscard]] static T Create(Args &&...args) { return T(std::forward<Args>(args)...); }
};
...
namespace Detail
{
template <typename To, typename From> struct CastDispatcher
{
    static constexpr bool is_same = std::is_same_v<std::decay_t<To>, std::decay_t<From>>;
    static constexpr bool is_both_arithmetic =
        std::is_arithmetic_v<std::decay_t<To>> && std::is_arithmetic_v<std::decay_t<From>>;
...
template <typename To, typename From> [[nodiscard]] constexpr decltype(auto) TECast(From &&from) noexcept
{
    ...
    else if constexpr (Detail::CastDispatcher<To, From>::is_enum_to_int ||
                       Detail::CastDispatcher<To, From>::is_int_to_enum)
    {
        return static_cast<To>(from);
    }
...
template <typename To> struct TECastCheck
{
    template <typename From> [[nodiscard]] static constexpr TE::TEResult<To, std::string> Try(From from) noexcept
    {
