...
public:
    ...
    static bool IsMouseInViewport(TEVector2 min, TEVector2 max)
    {
    ...
    static void DrawBox(TimeGUI::TimeGUIDrawList dl, TEVector2 c, TEVector2 sz, float rot, unsigned int col)
    {
        ...
        auto AddCorner = [&](TEVector2 center, float startAngle)
        {
            for (int i = 0; i <= segments; i++)
            {
                ...
                points.push_back(TEVector2(c.x + lx * cR - ly * sR, c.y + lx * sR + ly * cR));
            }
        };

        float hx = sz.x * 0.5f - rounding;
        float hy = sz.y * 0.5f - rounding;
        AddCorner(TEVector2(hx, hy), 0.0f);
        AddCorner(TEVector2(-hx, hy), 3.14159265f * 0.5f);
        AddCorner(TEVector2(-hx, -hy), 3.14159265f);
        AddCorner(TEVector2(hx, -hy), 3.14159265f * 1.5f);
                               ...
                               float thickness)
    {
        ...
        dl.AddPolyline(points.data(), (int)points.size(), col, TimeGUIDrawFlags_Closed, thickness);
    }
    static void DrawTriangle(TimeGUI::TimeGUIDrawList dl, TEVector2 p1, TEVector2 p2, TEVector2 p3, unsigned int col)
    {
                            ...
                            unsigned int col)
    {
        ...
        if (std::abs(rot) > 0.001f)
        {
            float s = std::sin(rot);
            float c = std::cos(rot);
            auto Rotate = [&](TEVector2 p)
            {
                float dx = p.x - center.x;
                float dy = p.y - center.y;
                return TEVector2(center.x + dx * c - dy * s, center.y + dx * s + dy * c);
            };
            start = Rotate(start);
            end = Rotate(end);
        }
                                   ...
                                   unsigned int col, float thickness)
    {
        ...
        auto AddArc = [&](TEVector2 arcCenter, float startAngle)
        {
            for (int i = 0; i <= segments; i++)
            {
                ...
                points.push_back(TEVector2(center.x + lx * cR - ly * sR, center.y + lx * sR + ly * cR));
            }
        };

        AddArc(TEVector2(halfLen, 0), -3.14159265f * 0.5f);
        AddArc(TEVector2(-halfLen, 0), 3.14159265f * 0.5f);

        dl.AddPolyline(points.data(), (int)points.size(), col, TimeGUIDrawFlags_Closed, thickness);
    }

    static void DrawEllipse(TimeGUI::TimeGUIDrawList dl, TEVector2 c, float rx, float ry, float rot, unsigned int col)
    {
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            points[i] = TEVector2(c.x + lx * cR - ly * sR, c.y + lx * sR + ly * cR);
        }
                                ...
                                unsigned int oCol)
    {
        ...
        for (float dx = -thickness; dx <= thickness; dx += thickness)
        {
            for (float dy = -thickness; dy <= thickness; dy += thickness)
            {
                if (dx == 0 && dy == 0)
                    continue;
                dl.AddText(TEVector2(pos.x + dx, pos.y + dy), oCol, out.c_str());
            }
        }
    ...
    virtual void RegisterFunctions(std::vector<ProceduralFunc> &reg) override
    {
        auto GCol = [](const std::vector<float> &a, int s)
        {
            ...
            return TimeGUI::ColorConvertFloat4ToU32(TEVector4(a[s], a[s + 1], a[s + 2], a[s + 3]));
        };
        ...
        reg.push_back(
            {"TextOutline", "TextOutline(pos, value, thickness, outline_color, text_color)", "Renders outlined text.",
                ...
                const std::vector<std::string> &strings)
             {
                 if (args.size() >= 4)
                 {
                     unsigned int oCol = IM_COL32_BLACK, tCol = IM_COL32_WHITE;
                     if (args.size() >= 8)
        ...
        reg.push_back({"cos", "float cos(angle)", "Cosine function.", "Math", TEVector4(0.4f, 1, 0.6f, 1), nullptr,
                       [](const std::vector<float> &a, TEVector2 p, TEVector2 sz, float dt) -> std::vector<float>
                       { return {(a.size() >= 1) ? cos(a[0]) : 0.0f}; }});
        reg.push_back({"sin", "float sin(angle)", "Sine function.", "Math", TEVector4(0.4f, 1, 0.6f, 1), nullptr,
                       [](const std::vector<float> &a, TEVector2 p, TEVector2 sz, float dt) -> std::vector<float>
        ...
        reg.push_back({"IsMouseInViewport", "bool IsMouseInViewport()", "Returns true if mouse is inside simulator.",
                       "Utility", TEVector4(1, 0.4f, 0.8f, 1), nullptr,
                       [](const std::vector<float> &a, TEVector2 p, TEVector2 sz, float dt) -> std::vector<float>
                       {
                           TEVector2 m = TimeGUI::GetMousePos();
        ...
        reg.push_back({"IsMouseDown", "bool IsMouseDown()", "Returns true if left click is held.", "Utility",
                       TEVector4(1, 0.4f, 0.8f, 1), nullptr,
                       [](const std::vector<float> &a, TEVector2 p, TEVector2 sz, float dt) -> std::vector<float>
                       { return {TimeGUI::IsMouseDown(0) ? 1.0f : 0.0f}; }});
        reg.push_back({"HoverDistance", "float HoverDistance(pos, radius)", "Returns 0..1 based on mouse proximity.",
                       ...
                       [](const std::vector<float> &a, TEVector2 p, TEVector2 sz, float dt) -> std::vector<float>
                       {
                           if (a.size() < 3)
                               return {0};
                           TEVector2 m = TimeGUI::GetMousePos();
                           TEVector2 target = TEVector2(p.x + a[0], p.y + a[1]);
                           float r = a[2];
