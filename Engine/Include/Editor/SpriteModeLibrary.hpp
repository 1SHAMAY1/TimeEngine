...
public:
    ...
    virtual void RegisterFunctions(std::vector<ProceduralFunc> &reg) override
    {
        ...
        reg.push_back({"Capsule", "Capsule(p1, p2, rad, color)", "Renders a pill-shaped line.", "Primitives",
                       ImVec4(0.6f, 0.9f, 1, 1), [GCol](ImDrawList *dl, ImVec2 p, const std::vector<float> &args)
                       {
                           if (args.size() >= 9)
                               DrawCapsule(dl, ImVec2(p.x + args[0], p.y + args[1]),
                                           ImVec2(p.x + args[2], p.y + args[3]), args[4], GCol(args, 5));
                           else if (args.size() >= 5)
                               DrawCapsule(dl, ImVec2(p.x + args[0], p.y + args[1]),
                                           ImVec2(p.x + args[2], p.y + args[3]), args[4], IM_COL32_WHITE);
                       }});
