                            ...
                            float columnWidth = 100.0f)
{
    ...
    float itemWidth =
        (ImGui::GetContentRegionAvail().x - buttonSize.x * 3.0f - ImGui::GetStyle().ItemSpacing.x * 2.0f) / 3.0f;
                            ...
                            float columnWidth = 100.0f)
{
    ...
    ImVec2 buttonSize = {lineHeight + 3.0f, lineHeight};
    float itemWidth =
        (ImGui::GetContentRegionAvail().x - buttonSize.x * 2.0f - ImGui::GetStyle().ItemSpacing.x * 1.0f) / 2.0f;
                            ...
                            float columnWidth = 100.0f)
{
    ...
    ImVec2 buttonSize = {lineHeight + 3.0f, lineHeight};
    float itemWidth =
        (ImGui::GetContentRegionAvail().x - buttonSize.x * 4.0f - ImGui::GetStyle().ItemSpacing.x * 3.0f) / 4.0f;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        ImGui::SameLine();
        ImGui::SetNextItemWidth(itemWidth);
        if (ImGui::DragFloat((std::string("##") + labels[i]).c_str(), &values[i], 0.1f, 0.0f, 0.0f, "%.2f"))
            changed = true;
        if (i < 3)
            ImGui::SameLine();
    }
...
inline bool DrawColorControl(const std::string &label, glm::vec4 &values, float columnWidth = 100.0f)
{
    ...
    {
        if (ImGui::ColorPicker4("##Picker", &values.x,
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (ImGui::DragFloat(ids[i], &values[i], 0.01f, 0.0f, 1.0f, "%.2f"))
            changed = true;
        if (i < 3)
            ImGui::SameLine();
    }
...
{
    if (size == 0.0f)
        size = ImGui::GetFrameHeight() * 0.8f;
