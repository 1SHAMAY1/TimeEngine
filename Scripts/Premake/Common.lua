...
-- Dynamically detect MSVC Platform Toolset if MSVC is installed (zero hardcoded paths or version strings)
filter { "system:windows", "action:vs*" }
    local vcVer = os.getenv("VCToolsVersion") or ""
    if #vcVer == 0 then
    ...
    if #vcVer == 0 then
        local pipe = io.popen('vswhere.exe -latest -products * -requires Microsoft.Component.MSBuild -property installationPath 2>nul')
