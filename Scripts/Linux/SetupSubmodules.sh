...
# Initializes Git submodules and downloads Linux Premake 5 binary if missing.

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
ROOT_DIR="$( cd "$SCRIPT_DIR/../.." && pwd )"

echo "[=== Setting up TimeEngine Submodules & Tools (Linux) ===]"
cd "$ROOT_DIR"
...
# Premake 5 Linux Binary Download
