#!/bin/bash
# BuildCommercialRelease.sh (Linux Clang)
...
if [ ! -f "Makefile" ]; then
    echo "[!] Makefile not found. Please run Clang/GenerateProjectFiles.sh first."
    read -p "Press Enter to exit..."
    exit 1
fi

echo "[≡ Building project with Clang (Dist)...]"
make config=dist -j$(nproc 2>/dev/null || echo 4) CC=clang CXX=clang++
if [ $? -ne 0 ]; then
    echo "[✖ Build Failed!]"
    read -p "Press Enter to exit..."
    exit 1
