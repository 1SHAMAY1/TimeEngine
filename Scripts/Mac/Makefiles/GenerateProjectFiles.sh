#!/bin/bash
# GenerateProjectFiles.sh (Mac Makefiles)

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
ROOT_DIR="$( cd "$SCRIPT_DIR/../../.." && pwd )"

echo "[≡ Generating TimeEngine Makefiles: Clean + CMake + Premake]"
...
cmake ../glfw -DGLFW_BUILD_DOCS=OFF -DGLFW_BUILD_TESTS=OFF -DGLFW_BUILD_EXAMPLES=OFF -DCMAKE_BUILD_TYPE=Debug -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++
if [ $? -ne 0 ]; then
...
# Premake
echo "[≡ Generating GNU Makefiles with Premake...]"
cd "$ROOT_DIR"
...
echo "[✅ Setup complete. Run BuildDebug.sh to compile the project.]"
